...
{
	$tc=$_REQUEST["tc"];
	$ad=ucfirst($_REQUEST["secmen_ad"]);
	$soyad=ucfirst($_REQUEST["secmen_soyad"]);
	$dogum=$_REQUEST["secmen_dogum"];
	$tel=$_REQUEST["secmen_tel"];
	$adres=ucfirst($_REQUEST["secmen_adres"]);


	     $ozettc=hash('sha256',$tc);
	$ekle="INSERT INTO secmen (tc,ad,soyad,dogum_tarih,telefon,adres)
						VALUES('$ozettc','$ad','$soyad','$dogum','$tel','$adres')";
