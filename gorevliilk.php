...
$toplam1 = 0;
	$toplam2 = 0;
        ...
        $scc3 =  $toplam3 % 524309;
     $i=0;
            $query ="SELECT gorevli_id from gorevli";
         $gorevliler=$baglanti->query($query);
         foreach ($gorevliler as $key => $value) {
            $deger[$i] = $value['gorevli_id'];
            $i++;
        }
        $j=0;
    if ($_SESSION['secmen_id'] == $deger[$j]){
            echo "Gizli Değer 1: $scc1";
        }
        $j++;
        if ($_SESSION['secmen_id'] == $deger[$j]){
            echo "Gizli Değer 2: $scc2";
        }
         $j++;
            if ($_SESSION['secmen_id'] == $deger[$j]) {
            echo "Gizli Değer 3:  $scc3";
        }
?>
</body>
</html>