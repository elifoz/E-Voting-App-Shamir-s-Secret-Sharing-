...
$secmen_belirle=$_SESSION['secmen_id'];
   ...
   function intToBool($sayi,$sinir){
	...
	function secmenCC($katsayix2,$katsayix,$index,$encodeVote){
		...
		if(isset($_POST["oy_kullan"])){
              ...
              $durum_bilgi="SELECT durum from secmen where secmen_id ='$secmen_belirle'";
	$durum=$baglanti->query($durum_bilgi);
			foreach ($durum as $key => $value) {
				$secmen_oyu=$value[0];

			}
			if( $secmen_oyu == 1 and empty( $_POST['aday']) == 0){
	echo '<img src="img/if_101_Warning_183416.png">'."<h3>Birden Fazla Oy Kullanamazsınız!</h3>";
}

elseif (empty( $_POST['aday'])){
			echo '<img src="img/if_101_Warning_183416.png">'."<h3>Lütfen Desteklediğiniz Adayı Seçiniz!</h3>";
		}
		else	
		{
	$adaykim=$_POST['aday'];
	...
	$kul_oy=1;
	$guncelle= "UPDATE secmen SET durum ='$kul_oy' WHERE secmen_id ='$secmen_belirle'";
	  $s_calistir=$baglanti->query($guncelle); 
		echo '<img src="img/if_ok_61805.png">'."<h3>Oyunuz Alınmıştır!</br>Katılımınız İçin Teşekkürler.</h3>";
}
